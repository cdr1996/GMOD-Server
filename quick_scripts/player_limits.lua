...
hook.Add("PlayerCheckLimit", tag, function(ply, name, cur, max)
    -- Infinite limits for admins. This is subject to change,
    -- but for now lets have our fun.
    if ply:IsAdmin() then return false end

    if cur == ply:GetLimit(name) then
        ply:LimitHit(name) -- Notify the player they have reached their limit
        return true
    end
end)
...
hook.Add("PlayerInitialSpawn", tag, function(ply)
    if ply:GetUserGroup() == "user" then
        self.player_limits = default_limits
    end
end)